...
(function() {
	...
	function props() {
		...
		properties = {
			moz_and_webkit: [
			background + "-origin", background + "-size", border + "-image", border + "-image-outset", border + "-image-repeat", border + "-image-source", border + "-image-width", border + "-radius", box + "-shadow", column + "-count", column + "-gap", column + "-rule", column + "-rule-color", column + "-rule-style", column + "-rule-width", column + "-width"],

			moz_and_webkit_and_ms: [
			box + "-flex", box + "-orient", box + "-align", box + "-ordinal-group", box + "-flex-group", box + "-pack", box + "-direction", box + "-lines", box + "-sizing", animation + "-duration", animation + "-name", animation + "-delay", animation + "-direction", animation + "-iteration-count", animation + "-play-state", animation + "-timing-function", animation + "-fill-mode"],

			moz_and_webkit_and_ms_and_opera: [
			transform, transform + "-origin", transition, transition + "-property", transition + "-duration", transition + "-timing-function", transition + "-delay", "user-select"],

			misc: [
			background + "-clip", border + "-bottom-left-radius", border + "-bottom-right-radius", border + "-top-left-radius", border + "-top-right-radius"]
		}
		return properties;
	}
	...
	fx.processDec = function(originalRule, includeAllProperties) {
		...
		for (var r = 0; r < css_array.length; r++) {
			...
			if (inArray(property, prefixes01)) {
				...
				//-moz, -webkit, -ms
				new_rules.push(_moz + clean_rule, _webkit + clean_rule, (property == "box-align" ? _ms + property + ":middle": _ms + clean_rule));
				...
				if (property == __background + "-clip") {
					if (value === "padding-box") {
						new_rules.push(_webkit + clean_rule, _moz + property + ":padding");
					}
				} else {
					//Border-radius properties here ONLY
					var v = property.split("-");
					new_rules.push(_moz + "border-radius-" + v[1] + v[2] + ":" + value, _webkit + clean_rule);
				}
				...
				case "display":
					if (value == "box") {
						...
						new_rules.push("display:" + _moz + "inline-stack", "zoom:1;*display:inline");
					}
					break;
				case "text-overflow":
					if (value == "ellipsis") {
				...
				case "opacity":
					...
					new_rules.push(_ms + "filter:progid:DXImageTransform.Microsoft.Alpha(Opacity=" + opacity + ")", "filter: alpha(opacity=" + opacity + ")", _moz + clean_rule, _webkit + clean_rule);
					break;
				case __background + "-image": case __background + "-color": case __background:
	...
	module.exports = function (stylesheets) {
		var css_files = [];
		css_files.push(stylesheets);
		if (css_files.length) {
